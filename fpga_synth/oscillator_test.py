...
sine = [
    intbv(
        int(amp * 0.5 * (math.sin(2 * math.pi * x / n_samps) + 1)), min=0, max=amp + 1
    )
...
triangle = [
    intbv(
        n_samps // 2
        + int(
            2 * amp / (n_samps) * math.fabs(x % (n_samps) - (n_samps) / 2) - 2 * amp / 4
        ),
        min=0,
        max=amp + 1,
    )
    for x in range(n_samps // 4, 5 * n_samps // 4)
]

sawtooth = [
    intbv(
        int(amp * (x / n_samps - math.floor(0.5 + x / n_samps)) + amp // 2),
        min=0,
        max=amp + 1,
    )
    for x in range(n_samps)
